#!/bin/bash
# MoAI TeammateIdle Hook Wrapper - Generated by moai-adk
...
if command -v moai &> /dev/null; then
	exec moai hook teammate-idle < "$temp_file" 2>/dev/null
fi

# Try detected Go bin path from initialization
if [ -f "C:/Users/Nexsol/go/bin/moai" ]; then
	exec "C:/Users/Nexsol/go/bin/moai" hook teammate-idle < "$temp_file" 2>/dev/null
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
	exec "$HOME/go/bin/moai" hook teammate-idle < "$temp_file" 2>/dev/null
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
	exec "$HOME/.local/bin/moai" hook teammate-idle < "$temp_file" 2>/dev/null
fi
