...
# This script forwards stdin JSON to the moai hook subagent-start command.
...
if [ -f "C:/Users/Nexsol/go/bin/moai" ]; then
	exec "C:/Users/Nexsol/go/bin/moai" hook subagent-start < "$temp_file" 2>/dev/null
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
	exec "$HOME/go/bin/moai" hook subagent-start < "$temp_file" 2>/dev/null
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
	exec "$HOME/.local/bin/moai" hook subagent-start < "$temp_file" 2>/dev/null
fi
