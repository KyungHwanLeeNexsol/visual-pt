...
# This script forwards stdin JSON to the moai hook config-change command.
# Project-local hook: .claude/hooks/moai/handle-config-change.sh
...
if [ -f "C:/Users/Nexsol/go/bin/moai" ]; then
	exec "C:/Users/Nexsol/go/bin/moai" hook config-change < "$temp_file" 2>/dev/null
fi
...
if [ -f "$HOME/.local/bin/moai" ]; then
	exec "$HOME/.local/bin/moai" hook config-change < "$temp_file" 2>/dev/null
fi
